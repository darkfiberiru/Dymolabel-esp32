...
void  WiFiProvClass :: printQR(const char *name, const char *pop, const char *transport){
    ...
    // QR code generation disabled - use manual provisioning URL instead
    log_i("Copy paste the below URL in a browser for provisioning:");
    log_i("%s?data=%s", "https://espressif.github.io/esp-jumpstart/qrcode.html", payload);
    log_i("Or use the ESP BLE Provisioning app to scan for device: %s", name);
}
